...
//	get_next_streak(list);
	i = 0;
	while (i < list->size && ((is_sorted(list->cur, list->cur->next) || (list->cur->value == list->max && list->cur->next->value == list->min))))
	{
		list->cur = list->cur->next;
		i++;
	}
	return (i - list->size); // renvoie 0 si ok, renvoie autre chose si ko.
}
